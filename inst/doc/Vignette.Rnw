...
This guide tries to make few assumptions about the user's familiarity with the
...
\lstinline[breakatwhitespace=false]|C:\Program Files (x86)\Mplus\Mplus Examples\User's Guide Examples|. If you 
...
are located in: \lstinline[breakatwhitespace=false]|C:\Program Files (x86)\Mplus\Mplus Examples\Addendum Examples|.

To run this batch, enter this at the command line:

\lstinline|> runModels("C:/Program Files (x86)/Mplus/Mplus Examples/Addendum Examples")|

Note that you need to use forward slashes ("/"), not backslashes("\textbackslash")
in the path name. Alternatively, you could use double backslashes (e.g., \lstinline|"C:\\Program Files (x86)"| etc.).
...
\begin{lstlisting}
summaryStats <- extractModelSummaries("C:/Program Files (x86)/Mplus/Mplus Examples/User's Guide Examples/Outputs", filefilter="ex4.*")
\end{lstlisting}
...
As of this version of the package (0.2-4), the following summary statistics are automatically extracted,
when available:
...
See the Sweave manual for more details.
