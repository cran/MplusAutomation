...
As of this version of the package (0.3), the following summary statistics are automatically extracted,
when available:
...
the MODEL RESULTS section of a given Mplus output file.
Parameters include the parameter estimate, std. err, param/s.e., and two-tailed p-value.
Model results are returned as a \lstinline|data.frame| object.

Example:

\begin{lstlisting}
modelResults <- extractModelParameters("C:/Data_Analysis/Mplus Output.out", "stdyx")
\end{lstlisting} 

This call will return a data.frame object with all model parameters from the STDYX
standardization section of the output.

This function returns a \lstinline|data.frame| containing the model parameters for the section specified above. Variables include
\begin{enumerate}
...
w2name = Paran2 Szoid2 Sztyp2 Anti2 Border2 Hist2
	Narc2 Avoid2 Depend2 OCPD2;
w3name = Paran3 Szoid3 Sztyp3 Anti3 Border3 Hist3
