...
The package was built using R 2.12.0, although it should be compatible with
older versions of R. To obtain R 2.12.0 for Windows, follow this link:
\url{http://cran.r-project.org/bin/windows/base/R-2.12.0-win.exe}
...
As of version 0.4, a new function, \lstinline|readModels| is provided that wraps three functions: \lstinline|extractModelSummaries|,
\lstinline|extractModelParameters|, and \lstinline|getSavedata_Data|. The \lstinline|readModels| function will extract model summaries,
parameters, and saved data (using the Mplus SAVEDATA syntax), respectively, into a single list. The top-level elements of the list
represent distinct output files and are named by the corresponding Mplus output filename. Each top-level element is composed of up to three
sub-elements: \lstinline|summaries|, \lstinline|parameters|, and \lstinline|savedata|. The structure of each of these elements is the same
